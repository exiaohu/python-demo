...
async def test_create_user_by_superuser(
    ...
    r = await client.post(f"{settings.API_V1_STR}/users/", headers=superuser_token_headers, json=data)
...
@pytest.mark.asyncio
async def test_read_user_me(client: AsyncClient, superuser_token_headers: dict[str, str]) -> None:
